        ...
        if (!forecastArray.isNull()) {
            ...
            for (int i = 0; i < 3 && i < forecastArray.size(); i++) {
                ...
                Serial.println("  " + forecast[i].date + ": " + 
                    String(forecast[i].high) + "/" + String(forecast[i].low) + "°C " + 
...
void drawSmallWeatherIcon(int x, int y, String condition) {
    ...
    // Mist/Fog
    else if (condition.indexOf("mist") >= 0 || condition.indexOf("fog") >= 0 || condition.indexOf("haze") >= 0) {
        for (int i = 0; i < 4; i++) {
...
void drawWeatherIcon(int x, int y, String iconCode) {
    ...
    // Snow (13d, 13n)
        ...
        for (int i = 0; i < 6; i++) {
            ...
            display.drawLine(mx, my, mx + cos(angle + PI/6) * 15, my + sin(angle + PI/6) * 15, GxEPD_BLUE);
            display.drawLine(mx, my, mx + cos(angle - PI/6) * 15, my + sin(angle - PI/6) * 15, GxEPD_BLUE);
        }
...
void displayPrayerTimes() {
    ...
    do {
        ...
        String prayerTimesArr[] = {
            prayerTimes.fajr, prayerTimes.shuruq, prayerTimes.dhuhr,
            prayerTimes.asr, prayerTimes.maghrib, prayerTimes.isha
        };
        
        for (int i = 0; i < 6; i++) {
            int rowY = listStartY + i * rowHeight;
            int rowCenterY = rowY + rowHeight / 2 - 4;  // Vertical center of row (-4 to account for divider offset)
            
            // Divider line above each item (except first)
            if (i > 0) {
                display.drawLine(sectionX, rowY - 8, sectionX + sectionWidth, rowY - 8, GxEPD_BLACK);
            }
        ...
        // Degree symbol
        display.drawCircle(weatherCenterX - textWidth / 2 + 58, weatherStartY + 117, 5, GxEPD_BLACK);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            display.drawRoundRect(boxX, forecastY, boxWidth, boxHeight, r, GxEPD_BLACK);
            display.drawRoundRect(boxX + 1, forecastY + 1, boxWidth - 2, boxHeight - 2, r - 1, GxEPD_BLACK);
